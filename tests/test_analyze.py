...
def test_GeneratePtsMap_no_pos_fields():
    ...
    cutLocations = [
        (
            ...
            {'ptsTime': 100.5, 'sad': 0.9},
            {'ptsTime': 101.0, 'sad': 0.2},
        ),
    ]
    ...
    assert 100.5 in ptsmap
    assert ptsmap[100.5]['prev_end_pts'] == 99.0
    assert ptsmap[100.5]['next_start_pts'] == 101.0
...
def test_SplitVideo_uses_ffmpeg_with_pts():
    ...
    ptsmap = {
        ...
        "100.5": {"prev_end_pts": 99.0, "next_start_pts": 101.0, "prev_end_sad": 0.1, "next_start_sad": 0.2},
    ...
    try:
        with patch('tscutter.common.subprocess.run') as mock_run:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                assert ss_idx < i_idx, '-ss must be before -i'
                assert to_idx < i_idx, '-to must be before -i'
